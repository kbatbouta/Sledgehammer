...
public abstract class MongoDocument {
  ...
  /**
   * Adds a MongoDocumentEntry to the document. This is not automatically handled when the document
   * is loaded because of the concept of sub-classing document entries. This should be handled when
   * loading the document. The entries, however will save if they are added when the document is
   * loaded.
   *
  ...
  /**
   * Removes a MongoDocumentEntry from the document with a given String name. This also saves the
   * document.
   *
  ...
  public boolean removeEntry(String entryName) {
    ...
    // then this is an illegal situation. Throw the error.
      ...
      throw new IllegalArgumentException(
          "Entry does not exist for instance of \""
              + getClass().getName()
              + "\": \""
              + entryName
              + "\".");
