...
public class SledgehammerDatabase extends MongoDatabase {
  ...
  public MongoPlayer getMongoPlayer(String username, String password) {
    ...
    if (player == null) {
      DBCursor cursor =
          collectionPlayers.find(
              new BasicDBObject("username", username).append("password", MD5.encrypt(password)));
      if (cursor.hasNext()) {
        player = new MongoPlayer(collectionPlayers, cursor.next());
        registerPlayer(player);
      }
   ...
   */
  public MongoBan createBan(
      String id, String username, String reason, boolean steam, boolean banned) {
