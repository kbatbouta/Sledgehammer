...
public class MongoPlayer extends MongoUniqueDocument {
  ...
  public MongoPlayer(MongoCollection collection, UUID uuid) {
    ...
    reset();
    DBCursor cursor =
        collection.getDBCollection().find(new BasicDBObject(getFieldId(), getFieldValue()));
    if (cursor.hasNext()) {
      onLoad(cursor.next());
    }
    cursor.close();
  ...
  /**
   * @return Returns the nickname if valid. If the nickname is not valid, use the username instead.
   */
   ...
   * (Private method)
   *
   * <p>Sets the username for the player.
   *
