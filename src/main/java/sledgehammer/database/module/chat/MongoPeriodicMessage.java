...
public class MongoPeriodicMessage extends MongoDocument {
  ...
  /**
   * Updates the periodic message. Handles executing the message, and checking the time between
   * executions.
   */
  public void update() {
    ...
    if (isEnabled()) {
      ...
      if (timeNow - timeThen >= (time * 60000)) {
        ...
        if (shouldBroadcast()) {
          ...
          // Send it in-chat.
          ModuleChat module = SledgeHammer.instance.getPluginManager().getModule(ModuleChat.class);
  ...
  /** @return Returns whether or not to save to the database. */
  public boolean shouldSave() {
    return save;
  }

  /** @param flag Returns whether or not this message is saved to the database. */
