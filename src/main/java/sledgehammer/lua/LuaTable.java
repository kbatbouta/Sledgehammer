 ...
 * <p>TODO: document.
 *
 * <p>TODO: Redesign to pass KahluaTable through 'onExport()'. The current model is not efficient
 * with data storage.
 *
 * @author Jab
 */
public abstract class LuaTable extends LuaObject {
  ...
  public void copy(LuaTable other, byte flag) {
    ...
    if (!getName().equalsIgnoreCase(other.getName())) {
      errln(
          "LuaObject Class cannot be copied, because given class is different:"
              + "(LuaObject: \""
              + getName()
              + "\", Given: \""
              + other.getName()
   ...
   * @return Returns a field's data. If no Object is defined with the given Object key, null is
   *     returned.
   */
  ...
  /**
   * @return Returns whether or not the LuaTable is considered dirty (Data has changed without
   *     saving it).
   */
