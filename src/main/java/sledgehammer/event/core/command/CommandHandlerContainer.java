 ...
 * invocation is through the MethodHandle API introduced in the JDK 1.7 version in order to handle
 * methodical invocation that pre-compiles the data required to execute the method using the
 * traditional reflection API, allowing the execution of the command handler to execute with the
 * least amount of computations possible in the JVM.
 *
 * <p>Command handlers must contain one parameter for the method, which is the command being
 * interpreted and handled. If the parameters are not setup to only have the command being handled
 * and the response object, the container will not be enabled. The handler will still be registered
 * for debugging purposes.
 *
 * <p>An example command handler is as follows:
 *
 * <p><code>
...
public class CommandHandlerContainer {
   ...
   * Main constructor.
   *
   * @param container The Object to identify with the command handler. This is typically the
   *     declaring class.
  ...
  /**
   * Handles a command. If the command handler is not enabled, then we return without attempting to
   * handle the command.
   *
   * @param command The command to handle.
   * @param response The response to set.
   * @throws Throwable Thrown if the command handler fails to handle the command, or the method
   *     handle fails to invoke the method.
   */
   ...
   * (Private Method)
   *
   * <p>Sets the method that is the command handler being invoked when handling commands.
   *
   ...
   * (Private Method)
   *
   * <p>Sets the annotation instance for the command handler.
   *
  ...
  /**
   * @return Returns the UNIX timestamp for the container to identify when the container is created.
   */
   ...
   * (Private Method)
   *
   * <p>Sets the UNIX timestamp for the container to identify when the container is created.
   *
