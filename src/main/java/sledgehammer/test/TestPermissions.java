...
public class TestPermissions extends TestModule<ModulePermissions> {
  ...
  public TestPermissions() {
    ...
    setModule(
        (ModulePermissions)
            SledgeHammer.instance.getPluginManager().getModule(ModulePermissions.class));
  }
  ...
  private void cleanUp() {
    ...
    try {
      ...
      errln("Failed to delete User1.");
    }
    try {
      deleteUser2();
    } catch (Exception e) {
      errln("Failed to delete User2.");
    }
    try {
      deleteGroup1();
    } catch (Exception e) {
      errln("Failed to delete User3.");
    }
    try {
      deleteGroup2();
    } catch (Exception e) {
      errln("Failed to delete User4.");
    }
  }
   ...
   * (Private Method)
   *
   * <p>Create a PermissionUser, then delete it.
   */
  private void test1() {
    try {
   ...
   * (Private Method)
   *
   * <p>Create a PermissionGroup, then delete it.
   */
  private void test2() {
    try {
   ...
   * (Private Method)
   *
   * <p>Create a PermissionUser, a PermissionGroup and assign the user to that group. Remove the
   * group from the user, and then delete the group and user.
   */
  private void test3() {
    try {
      println(
          "Running Test 3: Assign PermissionUser to PermissionGroup."
   ...
   * (Private Method)
   *
   * <p>Tests PermissionGroup inheritance by creating 2 groups and 1 PermissionUser. Group 1 will be
   * assigned the parent for group 2. The user will be assigned to group 2. Group 2 will be deleted.
   * Then the other permission objects will be deleted.
   */
  private void test4() {
    try {
   ...
   * (Private Method)
   *
   * <p>Tests PermissionUser permission assignment.
   */
  private void test5() {
    boolean flag;
    try {
