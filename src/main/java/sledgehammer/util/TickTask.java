...
/**
 * Class to handle the execution of Tasks that are either one-time executions, delayed executions,
 * or timed executions.
 *
 * @author Jab
 */
public abstract class TickTask extends Printable {
  ...
  /**
   * Executes a TickTask implementation directly. This is called from the Sledgehammer engine
   * directly, and should not be ran by a third-party, although it can be used in this manner.
   *
   * <p>If the TickTask is a timer-task and an Exception is thrown, then the TickTask is cancelled.
   *
   * @return Returns true if the task is a timer-task and has successfully ran. If false is
   *     returned, the TickTask is unregistered from the TaskManager.
   */
  public boolean runTask() {
    ...
    // If the TickTask fails to execute, then make sure that ThrowableListeners pick it up by
    // stackTracing.
   ...
   *
   * <p>Throws an IllegalArgumentException if the following occurs: - The Module provided is null or
   * not loaded.
   *
   * <p>Throws an IllegalStateException if the following occurs: - The TickTask is already running.
   *
   ...
   *
   * <p>Throws an IllegalArgumentException if the following occurs: - The Module provided is null or
   * not loaded. - The delayTicksStart argument is less than 0.
   *
   * <p>Throws an IllegalStateException if the following occurs: - The TickTask is already running.
   *
   * @param module The Module to register the TickTask to the TaskManager.
   ...
   *
   * <p>Throws an IllegalArgumentException if the following occurs: - The Module provided is null or
   * not loaded. - The delayTicksStart argument is less than 0. - The delayTicksTimer argument is
   ...
   *
   * <p>The method throws an IllegalStateException if the TickTask is not running.
   */
  public void cancel() {
   ...
   *
   * @return The only importance of returning a boolean is for timer-tasks. If true is returned, the
   *     timer will keep running. If false is returned, the TickTask will be unregistered from the
   *     TaskManager.
   */
