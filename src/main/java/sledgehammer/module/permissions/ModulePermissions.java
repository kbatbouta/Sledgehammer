...
public class ModulePermissions extends MongoModule {
  ...
  /** The default PermissionGroup to base decisions for players not assigned to groups. */
   ...
   * (Private Method)
   *
   * <p>Loads the MongoDocuments, and the Lua containers for permission groups.
   */
  private void loadPermissionGroups() {
    ...
    while (cursor.hasNext()) {
      ...
      MongoPermissionGroup mongoPermissionGroup =
          new MongoPermissionGroup(collectionGroups, cursor.next());
    ...
    MongoPermissionGroup mongoPermissionGroupDefault =
        new MongoPermissionGroup(collectionGroups, "(Default)");
   ...
   * (Private Method)
   *
   * <p>Loads the MongoDocuments, and the Lua containers for permission users.
   */
  private void loadPermissionUsers() {
    ...
    while (cursor.hasNext()) {
      // Wrap the document in the proper container object.
      MongoPermissionUser mongoPermissionUser =
          new MongoPermissionUser(collectionUsers, cursor.next());
      // Assign it to the map for later reference.
   ...
   * (Private Method)
   *
   * <p>Pairs up PermissionUser to PermissionGroup and vice versa.
   */
        ...
        if (group == null) {
          errln(
              "PermissionUser \""
                  ...
                  + "\""
                  + "is assigned to a group that does not exist: \""
                  + groupId.toString()
                  + "\".");
   ...
   *
   * <p>Attempts to create a PermissionUser.
   *
   * @param commander The Player that sent the command.
   * @param username The Player's user-name.
   * @return Returns a Response that contains the Result of the Command, and the details for that
   *     Result.
   */
  public Response commandCreatePermissionUser(Player commander, String username) {
    ...
    try {
      ...
      response.set(
          Result.SUCCESS,
          lang.getString("command_permissions_user_create_success", language, fieldPlayer));
    } catch (Exception e) {
      stackTrace(e);
      response.set(
          Result.FAILURE,
          lang.getString("command_permissions_user_create_failure", language, fieldPlayer));
    }
    return response;
  }
   ...
   * (CommandListener Method)
   *
   * <p>Attempts to delete a PermissionGroup.
   *
   * @param commander The Player that sent the command.
   * @param username The Player's user-name.
   * @return Returns a Response that contains the Result of the Command, and the details for that
  ...
  public Response commandDeletePermissionUser(Player commander, String username) {
    ...
    if (permissionUser == null) {
      response.set(
          Result.FAILURE, lang.getString("permission_user_not_found", language, fieldPlayer));
      return response;
    }
    try {
      ...
      response.set(
          Result.SUCCESS,
          lang.getString("command_permissions_user_delete_success", language, fieldPlayer));
    } catch (Exception e) {
      stackTrace(e);
      response.set(
          Result.FAILURE,
          lang.getString("command_permissions_user_delete_failure", language, fieldPlayer));
    }
    return response;
  }
   ...
   * (CommandListener Method)
   *
   * <p>Sets a Permission Node definition for a PermissionUser.
   *
   * @param commander The Player that sent the command.
   * @param username The Player's user-name.
   ...
   *     Result.
   */
  public Response commandSetPermissionUserNode(
      Player commander, String username, String node, String flag) {
    // The Response to return.
    Response response = new Response();
    // Grab the Language set by the Player.
    Language language = commander.getLanguage();
    ...
    try {
      ...
      if (remove) {
        response.set(
            Result.SUCCESS,
            lang.getString(
                "command_permissions_user_set_node_success_remove",
                language,
                fieldPlayer,
                fieldNode));
      } else {
        response.set(
            Result.SUCCESS,
            lang.getString(
                "command_permissions_user_set_node_success",
                language,
                fieldPlayer,
                fieldNode,
                fieldFlag));
      ...
      if (remove) {
        response.set(
            Result.FAILURE,
            lang.getString(
                "command_permissions_user_set_node_failure_remove",
                language,
                fieldPlayer,
                fieldNode));
      } else {
        response.set(
            Result.FAILURE,
            lang.getString(
                "command_permissions_user_set_node_failure",
                language,
                fieldPlayer,
                fieldNode,
                fieldFlag));
   ...
   * (CommandListener Method)
   *
   * <p>Sets a PermissionUser's PermissionGroup.
   *
   * @param commander The Player that sent the command.
   * @param username The Player's user-name.
   ...
   *     Result.
   */
  public Response commandSetPermissionUserGroup(
      Player commander, String username, String permissionGroupName) {
    // The Response to return.
    Response response = new Response();
    // Grab the Language set by the Player.
    Language language = commander.getLanguage();
    ...
    PermissionGroup permissionGroupPrevious =
        permissionUser.hasPermissionGroup() ? permissionUser.getPermissionGroup() : null;
    ...
    if (!remove) {
      ...
      if (permissionGroup == null) {
        response.set(
            Result.FAILURE, lang.getString("permissions_group_not_found", language, fieldName));
        return response;
      }
    }
    ...
    if (!remove && permissionGroup.equals(permissionGroupPrevious)) {
      response.set(
          Result.FAILURE,
          lang.getString("permission_user_group_identical", language, fieldPlayer, fieldName));
      return response;
    }
    // Make sure that the player has a PermissionGroup to remove.
      ...
      response.set(
          Result.FAILURE,
          lang.getString("permission_user_group_already_not_set", language, fieldPlayer));
    }
    try {
      ...
      response.set(
          Result.SUCCESS,
          lang.getString(
              "command_permissions_user_set_group_success", language, fieldPlayer, fieldName));
    } catch (Exception e) {
      stackTrace(e);
      response.set(
          Result.FAILURE,
          lang.getString(
              "command_permissions_user_set_group_failure", language, fieldPlayer, fieldName));
    }
    return response;
  }
   ...
   * (CommandListener Method)
   *
   * <p>Attempts to create a PermissionGroup with the given name.
   *
   * @param commander The Player that sent the command.
  ...
  public Response commandCreatePermissionGroup(Player commander, String permissionGroupName) {
    ...
    try {
      // Create the document.
      MongoPermissionGroup mongoPermissionGroup =
          new MongoPermissionGroup(collectionGroups, permissionGroupName);
      ...
      response.set(
          Result.SUCCESS,
          lang.getString("command_permissions_group_create_success", language, fieldName));
    } catch (Exception e) {
      stackTrace(e);
      response.set(
          Result.FAILURE,
          lang.getString("command_permissions_group_create_failure", language, fieldName));
    }
    return response;
  }
   ...
   * (CommandListener Method)
   *
   * <p>Attempts to delete a PermissionGroup with the given name.
   *
   * @param commander The Player that sent the command.
  ...
  public Response commandDeletePermissionGroup(Player commander, String permissionGroupName) {
    ...
    try {
      ...
      response.set(
          Result.SUCCESS,
          lang.getString("command_permissions_group_delete_success", language, fieldName));
    } catch (Exception e) {
      stackTrace(e);
      response.set(
          Result.FAILURE,
          lang.getString("command_permissions_group_delete_failure", language, fieldName));
    }
    return response;
  }
   ...
   * (CommandListener Method)
   *
   * <p>Attempts to rename a PermissionGroup with the given name.
   *
   * @param commander The Player that sent the command.
   ...
   *     Result.
   */
  public Response commandRenamePermissionGroup(
      Player commander, String permissionGroupName, String permissionGroupNameNew) {
    // The Response to return.
    Response response = new Response();
    // Grab the Language set by the Player.
    Language language = commander.getLanguage();
    ...
    try {
      ...
      response.set(
          Result.SUCCESS,
          lang.getString(
              "command_permissions_group_rename_success", language, fieldName, fieldNameNew));
    } catch (Exception e) {
      stackTrace(e);
      // Set the failure message.
      response.set(
          Result.FAILURE,
          lang.getString("command_permissions_group_rename_failure", language, fieldName));
    }
    return response;
  }
   ...
   * (CommandListener Method)
   *
   * <p>Attempts to set the parent of a PermissionGroup.
   *
   * @param commander The Player that sent the command.
   * @param permissionGroupName The name of the PermissionGroup to rename.
   ...
   *     Result.
   */
  public Response commandSetPermissionGroupParent(
      Player commander, String permissionGroupName, String permissionGroupNameParent) {
    // The Response to return.
    Response response = new Response();
    // Grab the Language set by the Player.
    Language language = commander.getLanguage();
    ...
    if (!permissionGroupNameParent.equalsIgnoreCase("none")) {
      ...
      if (permissionGroupParent == null) {
        response.set(
            Result.FAILURE,
            lang.getString("permissions_group_not_found", language, fieldNameParent));
        return response;
      }
    }
    // If the parent to set is none, and the PermissionGroup already has no parent, then let the
    // commanding Player
    // know.
    else if (!permissionGroup.hasParent()) {
      response.set(
          Result.FAILURE,
          lang.getString("permissions_group_parent_already_not_set", language, fieldName));
      return response;
    }
    ...
    if (permissionGroupParent != null && permissionGroup.equals(permissionGroupParent)) {
      response.set(
          Result.FAILURE,
          lang.getString(
              "permissions_group_parent_identical", language, fieldName, fieldNameParent));
      return response;
    }
    ...
    if (permissionGroupParent != null && permissionGroupParent.isChildOf(permissionGroupParent)) {
      response.set(
          Result.FAILURE,
          lang.getString("permissions_group_parent_cyclic", language, fieldName, fieldNameParent));
      return response;
    }
    try {
      ...
      response.set(
          Result.SUCCESS,
          lang.getString(
              "command_permissions_group_set_parent_success",
              language,
              fieldName,
              fieldNameParent));
    } catch (Exception e) {
      ...
      response.set(
          Result.FAILURE,
          lang.getString("command_permissions_group_set_parent_failure", language, fieldName));
    }
    return response;
  }
   ...
   * (CommandListener Method)
   *
   * <p>Sets a Permission Node definition for a PermissionGroup.
   *
   * @param commander The Player that sent the command.
   * @param permissionGroupName The name of the PermissionGroup to rename.
   * @param node The Permission Node to set.
   ...
   *     Result.
   */
  public Response commandSetPermissionGroupNode(
      Player commander, String permissionGroupName, String node, String flag) {
    // The Response to return.
    Response response = new Response();
    // Grab the Language set by the Player.
    Language language = commander.getLanguage();
    ...
    try {
      ...
      response.set(
          Result.SUCCESS,
          lang.getString(
              "command_permissions_group_set_node_success",
              language,
              fieldName,
              fieldNode,
              fieldFlag));
      ...
      response.set(
          Result.FAILURE,
          lang.getString(
              "command_permissions_group_set_node_failure",
              language,
              fieldName,
              fieldNode,
              fieldFlag));
  ...
  public Response commandListPermissionGroup(Player commander, String permissionGroupName) {
    ...
    if (groupNodes.size() > 0) {
      ...
      for (Node node : listGroupNodes) {
        ...
        builder.append(
            lang.getString(
                "permission_node", new EntryField("node", nodeName), new EntryField("flag", flag)));
        builder.append(" <LINE> ");
      }
    ...
    EntryField[] fields =
        new EntryField[] {
          fieldName, fieldUUID, fieldUsers, fieldGroupPermissions,
        };
    response.set(
        Result.SUCCESS, lang.getString("command_permissions_group_info", language, fields));
    return response;
  }
  ...
  public Response commandListPermissionUser(Player commander, String username) {
    ...
    EntryField[] fields =
        new EntryField[] {
          fieldPlayer, fieldUUID, fieldName, fieldUserPermissions, fieldGroupPermissions
        };
  ...
  /**
   * Deletes a PermissionUser. If the PermissionUser is assigned to a PermissionGroup, it will be
   * removed from the list of members in that group.
   *
   * @param permissionUser The PermissionUser to delete. If it is null, IllegalArgumentException is
   *     thrown. If the PermissionUser isn't registered properly, a IllegalStateException is thrown.
   */
   ...
   *
   * @param name The String name of the PermissionGroup. If this is null, empty, or if the name is
   *     already in use, an IllegalArgumentException is thrown.
  ...
  /**
   * Deletes a PermissionGroup. If the group deleted is the parent of other groups, then the groups
   * are assigned to the parent of the group being deleted. If the group does not have a parent,
   * then the children groups will be unassigned and have no parent.
   *
  ...
  public void deletePermissionGroup(PermissionGroup permissionGroup) {
    ...
    UUID groupId = permissionGroup.getUniqueId();
    // If this is not a key in either map, something is wrong. Throw an exception.
    if (!mapMongoPermissionGroups.containsKey(groupId)
        || !mapPermissionGroups.containsKey(groupId)) {
      throw new IllegalStateException(
   ...
   * @return Returns a PermissionGroup using the given String name, if one exists. If one does not
   ...
   * @return Returns a PermissionUser associated for the given Player, if registered in Permissions
   *     as a PermissionsUser.
   */
  ...
  /**
   * @param playerId The Unique ID of a Player linked to the PermissionUser being retrieved.
   * @return Returns a PermissionUser associated with the given Unique ID for a Player, if
   *     registered in Permissions as a PermissionUser.
   */
