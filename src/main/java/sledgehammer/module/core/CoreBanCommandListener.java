...
import static sledgehammer.util.ChatTags.NEW_LINE;

public class CoreBanCommandListener implements Listener {

    private HashMap<String, String> mapTooltips;
    ...
    CoreBanCommandListener(ModuleCore module) {
        ...
        mapTooltips.put("ban", "Bans a player. Flags:"
                + NEW_LINE + " -s: SteamID flag (No ID required, but must be online!) ex: /ban -U" +
                " \"username\" -s"
                + NEW_LINE + " -S: SteamID flag (ID required!) ex: /ban -S \"11330\""
                + NEW_LINE + " -U: Username flag (Required unless \"-S\" or \"-I\") ex: /ban -U " +
                "\"username\""
                + NEW_LINE + " -i: IP flag (No IP required, but must be online!)"
                + NEW_LINE + " -I: IP flag (IP required!) ex: /ban -I \"127.0.0.1\" (Note: " +
                "without -U given, To undo this ban, the IP will be manditory as an argument!)");
    ...
    private void onCommandBan(Command c, Response r) {
        ...
        if (args.length > 1) {
            ...
            for (int x = 0; x < args.length; x++) {
                ...
                if ((arg.startsWith("-U") || arg.startsWith("-u")) && argN != null && !argN
                        .startsWith("-")) {
                    bUsername = true;
                    username = argN;
                    x++;
                } else if ((arg.startsWith("-R") || arg.startsWith("-r")) && argN != null &&
                        !argN.startsWith("-")) {
                    reason = argN;
                    x++;
                ...
                } else if ((arg.startsWith("-S") || arg.startsWith("-s") || arg.startsWith("-I")
                        || arg.startsWith("-i")
                        || arg.startsWith("-U") || arg.startsWith("-u") || arg.startsWith("-R")
                        || arg.startsWith("-r"))
            ...
            if (bIP && IP != null && !IP.isEmpty()) {
                ...
                ServerWorldDatabase.instance.banIp(IP,
    ...
    private void unban(Command c, Response r) {
        ...
        for (int x = 0; x < args.length; x++) {
            ...
            if ((arg.startsWith("-U") || arg.startsWith("-u")) && argN != null && !argN
                ...
                SteamID = argN;
                x++;
            } else if ((arg.startsWith("-I") || arg.startsWith("-S") || arg.startsWith("-U") ||
                    arg.startsWith("-u"))
                    && (argN == null || argN.startsWith("-"))) {
                r.set(Result.FAILURE, lang.getString("tooltip_command_unban", language));
                return;
        ...
        if (bSteamID) {
            ServerWorldDatabase.instance.banSteamID(SteamID, username == null
                    || username.isEmpty() ? null : username, false);
            response = "SteamID unbanned.";
        }
        if (bIP) {
            ServerWorldDatabase.instance.banIp(IP, username == null
                    || username.isEmpty() ? null : username, null, false);
