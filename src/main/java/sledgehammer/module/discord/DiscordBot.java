...
/**
 * Class designed to handle JavaCord API executions for the ModuleDiscord module.
 *
 * @author Jab
 */
public class DiscordBot extends Printable
    implements FutureCallback<DiscordAPI>, MessageCreateListener {
  ...
  /**
   * Attempts to establish a connection to the Discord Services, using a given email address and a
   * password.
   */
  ...
  /** Connects the bot to Discord using a generated token from a previous connection. */
  ...
  /** @return Returns whether or not the DiscordBot instance connected using a stored token. */
  ...
  public void staff(String staffMember, String string) {
    if (string == null || string.isEmpty()) return;
    say(
        module.getSettings().getModeratorChannel(),
        false,
  ...
  public void onMessageCreate(DiscordAPI api, final Message message) {
    ...
    if (!output.isEmpty()) {
      ...
      if (ModuleDiscord.DEBUG) {
        println(
            "[DISCORD] -> #" + channelName + ": " + username + ": " + COLOR_WHITE + " " + output);
      }
      ...
      if (c != null) {
        ChatMessage chatMessage =
            module.createChatMessage("[Discord] " + username + " : " + output);
